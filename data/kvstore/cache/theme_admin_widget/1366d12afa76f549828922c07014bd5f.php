<?php exit(); ?>a:3:{s:5:"value";s:3933:"<ul class="MenuList MenuList_<?php echo $this->_vars['widgets_id']; ?> clearfix">
<?php $this->_env_vars['foreach'][wgtmenu]=array('total'=>count($this->_vars['data']),'iteration'=>0);foreach ((array)$this->_vars['data'] as $this->_vars['key'] => $this->_vars['item']){
 ...
 if( $this->_vars['setting']['max_leng'] && $this->_vars['key']>$this->_vars['setting']['max_leng'] ){  if( $this->_vars['item']['custom_url'] != '' ){ ?>
    ...
    <?php }else{ ?>
    <div><a href=" app=$item.app ctl=$item.ctl act=$item.act args=$item.params full=1 }>"  <?php if( $this->_vars['item']['target_blank'] == 'true' ){ ?>target="_blank"<?php } ?>><?php echo $this->_vars['item']['title']; ?></a></div>
    ...
    <?php }else{ ?>
    <li><a href=" app=$item.app ctl=$item.ctl act=$item.act args=$item.params full=1 }>" <?php if( $this->_vars['item']['target_blank'] == 'true' ){ ?>target="_blank"<?php } ?>><?php echo $this->_vars['item']['title']; ?></a></li>
...
";s:3:"ttl";i:0;s:8:"dateline";i:1437787266;}