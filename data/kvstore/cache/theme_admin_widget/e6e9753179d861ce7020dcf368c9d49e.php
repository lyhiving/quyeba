<?php exit(); ?>a:3:{s:5:"value";s:3458:"<style>
.class1{color:#ff9900;}
.class2{color:#0099ff;}
						...
						<?php $this->_vars["do"]='1';  if($this->_vars['data']['goodsdata']['goodsRows'])foreach ((array)$this->_vars['data']['goodsdata']['goodsRows'] as $this->_vars['key'] => $this->_vars['goodsdemo']){ ?>
								<li class="class-<?php echo $this->_vars['do']++; ?>">
										...
										<span>
											...
											<?php echo app::get('ectools')->model('currency')->changer_odr($this->_vars['goodsdemo']['goodsSalePrice'],$_COOKIE["S"]["CUR"],false,false,app::get('b2c')->getConf('system.money.decimals'),app::get('b2c')->getConf('system.money.operation.carryset')); ?></i>
											<i class="mktprice"><?php echo app::get('ectools')->model('currency')->changer_odr($this->_vars['goodsdemo']['goodsMarketPrice'],$_COOKIE["S"]["CUR"],false,false,app::get('b2c')->getConf('system.money.decimals'),app::get('b2c')->getConf('system.money.operation.carryset')); ?></i>
										</span>
								</li>
						<?php } ?>
				</ul>
</div>";s:8:"dateline";s:10:"1376924104";s:3:"ttl";s:1:"0";}