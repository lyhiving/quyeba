<?php exit(); ?>a:3:{s:5:"value";s:3096:"<style>
.class1{color:#ff9900;}
.class2{color:#0099ff;}
				...
				<ul class="switchable-content clearfix">
						<?php $this->_vars["do"]='1';  if($this->_vars['data']['goodsdata']['goodsRows'])foreach ((array)$this->_vars['data']['goodsdata']['goodsRows'] as $this->_vars['key'] => $this->_vars['goodsdemo']){ ?>
								<li class="class-<?php echo $this->_vars['do']++; ?>">
										...
										<span><?php echo $this->_vars['setting']['price_show']; ?>：<i><?php echo app::get('ectools')->model('currency')->changer_odr($this->_vars['goodsdemo']['goodsSalePrice'],$_COOKIE["S"]["CUR"],false,false,app::get('b2c')->getConf('system.money.decimals'),app::get('b2c')->getConf('system.money.operation.carryset')); ?></i></span>
								</li>
						<?php } ?>
				</ul>
