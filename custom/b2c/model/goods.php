...
/*                'tag'=>array(
    ...
    var $ioSchema = array(
	    ...
	    //author by charect 2013.4.22
            'col:english_title' => 'english_title',
	    'col:科技课堂' => 'extend',
	    'col:尺码表' => 'size',
...
//            return $this->ioTitle['csv'][$filter['type_id']];
        ...
        switch( $ioType ){
            ...
            default:
			 ...
			 //author by charect 2013.4.22
	   		app::get('b2c')->_('col:科技课堂') => 'extend',
			app::get('b2c')->_('col:尺码表') => 'size',
    ...
    //获取商品科技课堂 @author by charect 2013.4.27
    function get_goods_extend($gid){
        $sql = 'SELECT extend FROM sdb_b2c_goods WHERE goods_id = '.$gid;
        $query = $this->db->select($sql);
        foreach ($query as $extend){
            $extend;
        }
        return $extend;
    }
    
    //获取商品尺寸 @author bu charect 2013.4.27
    function get_goods_size($gid){
        $sql = 'SELECT size FROM sdb_b2c_goods WHERE goods_id = '.$gid;
        $query = $this->db->select($sql);
