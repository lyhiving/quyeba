...
class b2c_ctl_site_product extends b2c_frontpage{
    ...
    public function index() {
       ...
       $this->pagedata['specShowItems'] =$specValue;
        ...
        //商品折扣价 @author by charect
        
        $goodsPr = $this->app->model('goods_promotion_ref');
        $goodsSp = $db->select("SELECT g.`mktprice`,gpr.`goods_id`,gpr.action_solution FROM `sdb_b2c_goods_promotion_ref` AS gpr LEFT JOIN sdb_b2c_sales_rule_goods AS srg ON gpr.rule_id = srg.rule_id 
                                    LEFT JOIN sdb_b2c_goods AS g ON gpr.`goods_id` = g.`goods_id`
                                                                WHERE gpr.`goods_id`=".$gid."");
        $goodsSprice = unserialize($goodsSp['0']['action_solution']);
        $zhekou = $goodsSprice['b2c_promotion_solutions_topercent']['percent']/100;
        $goodsSaleprice = $goodsSp['0']['mktprice']*$zhekou;
        $this->pagedata['salesprice'] = $goodsSaleprice;
        $imageDefault = app::get('image')->getConf('image.set');
    ...
    */
    ...
    function goodsBodyContent($gid=0, $aGoods=null) {
        if(!$aGoods){
            ...
            $aGoods_row = $objGoods->getList('goods_id,intro,extend,size,price,cost,mktprice,type_id,brand_id,params,p_1,p_2,p_3,p_4,p_5,p_6,p_7,p_8,p_9,p_10,p_11,p_12,p_13,p_14,p_15,p_16,p_17,p_18,p_19,p_20,p_21,p_22,p_23,p_24,p_25,p_26,p_27,p_28,p_29,p_30,p_31,p_32,p_33,p_34,p_35,p_36,p_37,p_38,p_39,p_40,p_41,p_42,p_43,p_44,p_45,p_46,p_47,p_48,p_49,p_50',array('goods_id'=>intval($gid)));
	    ...
	    $aGoods['size'] = $aGoods_row[0]['size'];
	    $aGoods['extend'] = $aGoods_row[0]['extend'];//author by charect 2013.4.25
        }
	...
	$aGoods['extend'] = preg_replace("/(\<img[\s\S]+)src=/Us","\\1src='".$this->app->res_url."/images/transparent.gif' img-lazyload=",$aGoods['extend']);
        $aGoods['size'] = preg_replace("/(\<img[\s\S]+)src=/Us","\\1src='".$this->app->res_url."/images/transparent.gif' img-lazyload=",$aGoods['size']);

        /**** begin 商品评论 ****/
        $aComment['switch']['ask'] = $this->app->getConf('comment.switch.ask');
        $aComment['switch']['discuss'] = $this->app->getConf('comment.switch.discuss');
        //科技课堂 @author by charect
        $aComment['switch']['extend'] = $this->app->getConf('comment.switch.extend');
        //商品尺寸 @author by charect
         $aComment['switch']['size'] = $this->app->getConf('comment.switch.size');
        ...
        //科技课堂
        if($aComment['switch']['extend'] == "on") {
            $aComment['exntendCount'] = $this->app->model("member_comments")->count(array('object_type'=>'extend', 'display'=>'true', 'type_id'=>$gid));
        }
        //商品尺寸
        if($aComment['switch']['size'] == "on") {
            $aComment['exntendCount'] = $this->app->model("member_comments")->count(array('object_type'=>'size', 'display'=>'true', 'type_id'=>$gid));
        }
        ...
        $this->pagedata['extendshow'] = $this->app->getConf('comment.verifyCode.extend');
        /**** end 商品评论 ****/
    ...
    function goodsDiscuss($gid=0, $aGoods=null){
        ...
        $this->pagedata['discussshow'] = $this->app->getConf('comment.verifyCode.discuss');
        $this->pagedata['count_shaidan'] = app::get('shaidan')->model('shaidan')->count(array('goods_id'=>$gid,'display'=>'true'));
    ...
    //获得科技课堂 @author by charect 2013.4.26
    function goodsExtend($gid=0, $aGoods=null){
	$member_data = $this->get_current_member();
        if(!$member_data['member_id']){
            $this->pagedata['login'] = 'nologin';
        }
        $memInfo['member_id'] = $member_data['member_id'];
	$this->pagedata['extend_status'] = kernel::single('b2c_message_disask')->toValidate('extend',$gid,$memInfo,$extend_message);
        $this->pagedata['extend_message'] = $extend_message;
		
	$objExtend = $this->app->model('goods');
	$this->pagedata['extend'] = $objExtend->get_goods_extend($gid);
//        var_dump($this->pagedata['extend']);
        $this->pagedata['extendshow'] = $this->app->getConf('comment.verifyCode.extend');
        echo kernel::single("b2c_goods_description_comments")->show($gid,$aGoods,'extend', $this->custom_view);
    }
    
    //获得商品详情页商品尺寸 @author by charect 2013.4.27
     function goodsSize($gid=0, $aGoods=null){
	$member_data = $this->get_current_member();
        if(!$member_data['member_id']){
            $this->pagedata['login'] = 'nologin';
        }
        $memInfo['member_id'] = $member_data['member_id'];
	$this->pagedata['size_status'] = kernel::single('b2c_message_disask')->toValidate('size',$gid,$memInfo,$extend_message);
        $this->pagedata['size_message'] = $extend_message;
		
	$objExtend = $this->app->model('goods');
	$this->pagedata['size'] = $objExtend->get_goods_size($gid);
        $this->pagedata['extendshow'] = $this->app->getConf('comment.verifyCode.extend');
        echo kernel::single("b2c_goods_description_comments")->show($gid,$aGoods,'size', $this->custom_view);
    }
