...
class b2c_ctl_site_cart extends b2c_frontpage{
    ...
    public function index(){
        ...
        //用于omniture
        $carts_info = $this->mCart->get_basic_objects();
        $result = null;
        foreach($carts_info as $value) {
           // var_dump($value['params']['goods_id']);
            $result[] = $value['params']['goods_id'];
        }
        $this->pagedata['cart_product_id'] = ';'.implode(',;',$result);
    ...
    public function checkout_result($isfastbuy=0)
    {
        ...
        foreach ($this->pagedata['aCart']['object']['goods'] as $arrGoodsInfo)
        {
            ...
            //用于omniture
            $arr[] = $goods_id;
            $this->pagedata['checkout_product_id'] = ';'.implode(',;', $arr);
