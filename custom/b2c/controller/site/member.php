...
class b2c_ctl_site_member extends b2c_frontpage{
    ...
    protected function get_order_details(&$aData,$tml='member_orders')
    {
        if (isset($aData['data']) && $aData['data'])
        {
            ...
            foreach ($aData['data'] as $key=>&$arr_data_item)
            {
                $this->get_order_detail_item($arr_data_item,$tml);
                $gids = array();
                foreach($arr_data_item['goods_items'] as $p_k=>$product_row){
                    $gids[] = $product_row['product']['goods_id'];
                }
                $member_disscuss = $member_comment->getList('comment_id',array('for_comment_id' => 0,'object_type' => 'discuss','type_id' => $gids,'author_id' => $this->app->member_id));
                $comment_ids = array();
                if($member_disscuss){
                    foreach($member_disscuss as $comment_id){
                        $comment_ids[] = $comment_id['comment_id'];
                    }
                    $aData['data'][$key]['is_discuss'] = 'true';
                }else{
                    $aData['data'][$key]['is_discuss'] = 'false';
                }
                $shaidan = app::get('shaidan')->model('shaidan')->getList('s_id',array('comment_id'=>$comment_ids));
                if($shaidan){
                    $aData['data'][$key]['is_shaidan'] = 'true';
                }else{
                    $aData['data'][$key]['is_shaidan'] = 'false';
                }
    ...
    public function orderdetail($order_id=0)
    {
        ...
        $member_comment = $this->app->model('member_comments');
        foreach($sdf_order['goods_items'] as $p_k=>$product_row){
            $member_disscuss = $member_comment->getList('comment_id',array('for_comment_id' => 0,'object_type' => 'discuss','type_id' => $product_row['product']['goods_id'],'author_id' => $this->app->member_id));
            if($member_disscuss){
                $sdf_order['goods_items'][$p_k]['product']['is_discuss'] = 'true';
            }else{
                $sdf_order['goods_items'][$p_k]['product']['is_discuss'] = 'false';
            }
            $shaidan = app::get('shaidan')->model('shaidan')->getList('s_id',array('comment_id'=>$member_disscuss[0]['comment_id']));
            if($shaidan){
                $sdf_order['goods_items'][$p_k]['product']['is_shaidan'] = 'true';
            }else{
                $sdf_order['goods_items'][$p_k]['product']['is_shaidan'] = 'false';
            }
        }
        $this->pagedata['order'] = $sdf_order;
        $this->pagedata['switch_discuss'] = $this->app->getConf('comment.switch.discuss');
        $this->pagedata['power_discuss'] = $this->app->getConf('comment.power.discuss');
