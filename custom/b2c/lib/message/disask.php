...
class b2c_message_disask extends b2c_message_comment{
  ...
  ////读取商品评论回复列表
   function getCommentsReply($aId, $display=false){
    ...
    //获取商品咨询和评论包括回复
    function good_all_disask($gid=null,$page=1,$type_id=null,$is_shaidan=false){
        if(!$gid) return;
        ...
        $aComment['goods_discuss_notice'] = $this->app->getConf('comment.goods_discuss_notice');
         foreach($aComment['switch'] as $item => $switchStatus){
            if($switchStatus == 'on'){
                $commentList = $this->getGoodsIndexComments($gid,$item,$page,$type_id,$is_shaidan);
                if($is_shaidan && !$commentList){
                    return;
                }
    ...
    function getGoodsIndexComments($gid,$item,$page=1,$type_id=null,$is_shaidan=false){
        $list_listnum = intval($this->app->getConf('comment.index.listnum'));
        $this->objComment->type = $item;
        $filter['for_comment_id'] = 0;
        $filter['display'] = 'true';
        if($is_shaidan){
            $shaidan_data = app::get('shaidan')->model('shaidan')->getList('comment_id',array('goods_id'=>$gid,'display'=>'true'));
            if($shaidan_data){
                foreach($shaidan_data  as $shaidan_row){
                    $filter['comment_id'][] = $shaidan_row['comment_id'];
                }
            }else{
                return;
            }
        ...
        foreach((array)$aData as $key=>$val){
            ...
            if($val['object_type'] == 'discuss'){
                $shaidan = array();
                $shaidan = $this->getShaidanItem($val['comment_id']);
                $val['shaidan'] = $shaidan;
                $row = $goods_point->get_comment_point($val['comment_id']);
                $val['goods_point'] = $row;
                if(!$shaidan && $is_shaidan == 'true'){
                    $flag = 1;
                }
    ...
    function getShaidanItem($comment_id){
        $row = app::get('shaidan')->model('shaidan')->getList('*',array('comment_id'=>$comment_id,'display'=>'true'));
        if($row){
            $row[0]['image_info'] = unserialize($row[0]['image_info']);
            $data = $row[0];
            $data['imgnum'] = count($row[0]['image_info']);
        }else{
            $data = false;
        }
        return $data;
