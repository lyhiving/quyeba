...
class utils{
    ...
    static function cal_fee($exp,$weight,$totalmoney,$first_price,$continue_price,$defPrice=0){
		// 20130204 Andrew 俱乐部特价商品不包运费
		$aCart = kernel::single('b2c_mdl_cart')->get_objects();
		foreach ($aCart['object']['goods'] as $goods){
			$goods_cat = kernel::single('b2c_mdl_goods_cat')->dump($goods['obj_items']['products'][0]['cat_id']);
